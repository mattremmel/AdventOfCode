...
impl Challenge for Day3 {
        ...
        T: AsRef<str>,
    {
        input
            ...
            .map(|(lhs, rhs)| (to_bitset(lhs), to_bitset(rhs)))
            .map(|(lhs, rhs)| (lhs & rhs).trailing_zeros())
            .sum::<Priority>()
    }
        ...
        T: AsRef<str>,
    {
        input
            ...
            .lines()
            .tuples::<(_, _, _)>()
            .map(|(first, second, third)| (to_bitset(first), to_bitset(second), to_bitset(third)))
            .map(|(first, second, third)| (first & second & third).trailing_zeros())
            .sum::<Priority>()
    }
}

fn to_bitset(s: &str) -> u64 {
    s.chars().fold(0, |n, c| n.with_bit(to_priority(c)))
}

fn to_priority(c: char) -> Priority {
    match c.is_ascii_lowercase() {
        true => c as Priority - 96,
        false => c as Priority - 38,
    }
}
